<script>
	...
	// Define reusable data for social links
	const socialLinks = [
		{
			name: 'X',
			url: 'https://x.com/codingtogo',
			color: '#000',
			icon: `
				<svg style="height: 1.95rem;" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
					<title>X</title>
		...
		{
			name: 'YouTube',
			url: 'https://youtube.com/@codingtogo',
			color: 'red',
			icon: `
				<svg style="height: 1.95rem;" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
					<title>YouTube</title>
...
<div class="socials-container" bind:this={container} in:fade>
	{#each socialLinks as social}
		<a
			href={social.url}
			data-social={`My Guy Bri's ${social.name}`}
			style={`--accent-color: ${social.color};`}
			aria-label={`Visit My Guy Bri's ${social.name}`}
		>
			{@html social.icon}
		</a>
	{/each}
...
<style>
	...
	.socials-container a::before {
		...
		position: absolute;
		background-color: var(--accent-color);
		color: rgb(246, 254, 188);
		text-decoration: none;
		font-size: 0.85rem;
		padding: 0.5em 1em;
		border-radius: 100px;
		box-shadow: 0 5px 10px rgba(0, 0, 0, 0.05);
		transform: translateY(-29px) rotate(25deg);
		opacity: 0;
